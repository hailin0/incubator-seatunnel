...
public class DamengConnectorConfig extends HistorizedRelationalDatabaseConnectorConfig {
    ...
    public static final Field LOG_MINING_SLEEP_TIME_MIN_MS = Field.create("log.mining.sleep.time.ms")
        .withDisplayName("Sleep time in milliseconds when reading redo/archive logs.")
        .withType(ConfigDef.Type.LONG)
        .withWidth(ConfigDef.Width.SHORT)
        .withImportance(ConfigDef.Importance.LOW)
        .withDefault(1000)
        .withDescription(
            "The time that the connector will sleep after reading data from redo/archive logs and before starting reading data again. Value is in milliseconds.");
